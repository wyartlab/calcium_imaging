...
if hasTime == 0;
    ...
    [Mean, Err] = grpstats(combined(:,3)./time, combined(:,5), {'mean', 'sem'});
    ...
    jitter
    title('all cells')
    errorbar([condSet], Mean, Err, 'LineStyle', 'none', 'LineWidth', 2, 'Marker', '+');
...
elseif hasTime == 1;
    ...
    jitter
    errorbar(unique(temp(:,6)+offsetMat(i)), StatMat(:,1,i)', StatMat(:,2,i)', 'Color', colorMat(i,:));
    clear temp
    title('all cells')
    end
